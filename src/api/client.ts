...
export const api = {
  ...
  promptTemplates: () => requestJson<PromptTemplate[]>('/api/prompt-templates'),
  ...
  deletePromptTemplate: (id: string) =>
    requestJson<void>(`/api/prompt-templates/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    }),
  searchIconfont: (query: string) =>
    requestJson<IconfontSearchItem[]>(
      `/api/iconfont/search?q=${encodeURIComponent(query)}`,
    ),
